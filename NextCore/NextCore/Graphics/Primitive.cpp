...
namespace NextCore::Graphics
{
	...
	//		m_normals[i] = Normalize(Cross(line1, line2));
	//	}

	//	Vector3 cameraPosition = { 0, 0, 0 };

	//	Vector3 commonPointOnPrimitive1 = Vector3(vertexPositions[0]) - cameraPosition;
	//	Vector3 commonPointOnPrimitive2 = Vector3(vertexPositions[3]) - cameraPosition;

	//	if (Dot(m_normals[0], commonPointOnPrimitive1) < 0 || Dot(m_normals[1], commonPointOnPrimitive2) < 0)
	//	{
	//		Vector3 commonNormal = m_normals[0];

	//		if (m_normals[1] != Vector3(0))
	//		{
	//			commonNormal += m_normals[1];
